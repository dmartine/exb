import {React, DataSource, RepeatedDataSourceProvider, WidgetManager, DataRecord, DataSourceStatus, QueriableDataSource} from 'jimu-core';
...
export default class Widget extends BaseWidget<AllWidgetProps<Config>, State>{
  ...
  query = () => {
    ...
    let inheritDsId = this.props.useDataSources.find(uds => uds.isInherited).dataSourceId;
    if(this.props.repeatedDataSource && inheritDsId){
      let contextDss = Array.isArray(this.props.repeatedDataSource) ? this.props.repeatedDataSource : [this.props.repeatedDataSource];
      let contextDs = contextDss.find(cds => cds.dataSourceId === inheritDsId);
  ...
  componentDidUpdate(prevProps, prevState){
    ...
    (this.state.dataSource as QueriableDataSource).query(this.state.query).then(records => {
      this.setState({
        records: records,
        queryState: DataSourceStatus.Loaded
      });
    }, err => this.setState({queryState: DataSourceStatus.Error}));
  }
  ...
  dataRender = () => {
    let fName1 = this.props.useDataSources[0].fields[0];
    let fName2 = this.props.useDataSources[0].fields[1];
              ...
              <div style={{background: '#eee', margin: '2px'}}>
                ...
                <div style={{background: '#ddd', margin: '5px'}}>{
                  this.props.config.innerWidgetIds.map(wId => {
                    let Widget = WidgetManager.getInstance().getWidgetClass(wId);
                    if(Widget){
                      return <div style={{border: '1px solid red'}} key={wId}><Widget/></div>;
                    }else{

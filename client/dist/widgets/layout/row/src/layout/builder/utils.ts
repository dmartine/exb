import { Immutable, LayoutItemConstructorProps, LayoutInfo } from 'jimu-core';
...
export function snapLeft(
  ...
  allRects.some((cItem, index) => {
    ...
    if (mouseInCItem) {
      ...
      if (value >= cItem.left + cItem.width / 2 && value <= cItem.left + cItem.width) {
...
export function resizeItem(id: string, dx: number, dw: number, allRects: ChildRect[]): IMChildRect[] {
  ...
  if (dx > 0) { // resize at the left side, decrease width
  ...
  if (dx < 0) { // resize at the left side, increase width
    let previousSize = 0;
...
export function insertItem(rect: ChildRect, pos: number, allRects: ChildRect[]): IMChildRect[] {
  ...
  const checkpoint = () => {
    if (insertIndex === 0) {
      ...
      used = insertIndex < rects.length ? rects[insertIndex].left - (previous.left + previous.width) :

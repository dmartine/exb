import { getAppConfigAction, AppConfigAction, builderAppSync, appConfigUtils } from 'jimu-for-builder';
import { IMAppConfig, LayoutItemType, getAppStore, utils, appActions } from 'jimu-core';
import { Status } from './config';


export function selectSelf(props, fromPage?: boolean){
  const {appConfig, id, dispatch, browserSizeMode} = props;
  
  const layoutInfos = appConfigUtils.getWidgetOrSectionLayoutInfosInOneSizeMode(appConfig, id, LayoutItemType.Widget, browserSizeMode);
  if(layoutInfos){
    if(layoutInfos.length > 1){
      ...
      if(widgetId){
        ...
        if(widgetJson && widgetJson.manifest && widgetJson.manifest.name && widgetJson.manifest.name === 'list'){
          ...
          if(currentStatus){
            const currentLayoutId = utils.findLayoutId(widgetJson.layouts[currentStatus], browserSizeMode, appConfig.mainSizeMode);
...
export function handleResizeCard(props, newCardSize, isTop: boolean = false, isLeft: boolean = false, isEnd: boolean = false, appConfig: IMAppConfig = undefined): AppConfigAction{
  ...
  // const layoutInfos = appConfigUtils.getWidgetOrSectionLayoutInfosInOneSizeMode(appConfig, id, LayoutItemType.Widget);
...
//   const remIndex = style && style.toLocaleLowerCase().indexOf('rem');
//   const pxIndex = style && style.toLocaleLowerCase().indexOf('px');
//   if(remIndex > -1){
